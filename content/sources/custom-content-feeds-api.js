...
const resolve = function resolve(key) {
  ...
  const ansFields = [...defaultANSFields, 'content_elements', `websites.${key['arc-site']}`]
  // resolvers only support text or number, turn into bool.
  const sitemapAtRoot = key['Sitemap-at-root'] && key['Sitemap-at-root'].toLowerCase() !== 'false'
  ...
  if (key['Source-Include']) {
    key['Source-Include'].split(',').forEach(i => i && !ansFields.includes(i) && ansFields.push(i))
  }
  ...
  if (key['Include-Terms']) {
    console.log('aasdasdasdasd', key['Include-Terms'])
    try {
  ...
  body.query.bool.must.push({
    term: { canonical_website: key['arc-site'] },
  })
  ...
  const Section = key.Section && sitemapAtRoot ? key.Section.replace(/-/g, '/') : key.Section
